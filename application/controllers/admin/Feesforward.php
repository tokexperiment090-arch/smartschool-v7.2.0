...
class Feesforward extends Admin_Controller
{
        ...
        if (!empty($studentlist)) {
            ...
            $record_exists         = $this->studentfeemaster_model->getBalanceMasterRecord($this->balance_group, $student_session_array);
                ...
                foreach ($student_Array as $stkey => $eachstudent) {
					...
					$record_exists         = $this->studentfeemaster_model->getBalanceMasterRecordNew($this->balance_group, $eachstudent->student_session_id);
					if (!empty($record_exists)) {
						$eachstudent->yesorno = $this->lang->line('assigned');
					}else{
						$eachstudent->yesorno = "Not Assigned";
					}
                    //==========================
	...
	public function deletefeesforward()
    {         
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if ($action == 'search') {
                ...
                if ($this->form_validation->run() == true) {
                    $data['student_due_fee'] = array();
                    if (!empty($pre_session)) {
                        $student_Array = json_decode($this->findPreviousBalanceFeesNew($pre_session->id, $class_id, $section_id, $current_session));
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/feesforward/deletefeesforward', $data);
        $this->load->view('layout/footer', $data);
    }
	
	public function findPreviousBalanceFeesNew($session_id, $class_id, $section_id, $current_session)
    {
        $studentlist = $this->student_model->getPreviousSessionStudent($session_id, $class_id, $section_id);
    ...
    public function bulk_delete()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($students as $student_key => $student_value) {
                $this->studentfeemaster_model->deleteBalanceFees($student_value);
            }
